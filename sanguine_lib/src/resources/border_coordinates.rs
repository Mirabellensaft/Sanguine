...
impl BorderCoordinates {
    ...
    fn new(field: &layout::Field, amount: usize) -> Self {
        ...
        let x = array_of_values(field.x, field.column_width,1);
        for i in 0..amount {
            coordinates.0.push(Point::new(x[i], field.y as f32 + field.row_height as f32));
            // println!("coordinates:{:?}", coordinates[side][i]);
        }
        sides.push(coordinates);
        ...
        let y = array_of_values(field.y, field.row_height,1);
        for i in 0..amount {
            coordinates.0.push(Point::new(field.x as f32 + field.column_width as f32, y[i]));
            // println!("coordinates:{:?}", coordinates[side][i]);
        }
        sides.push(coordinates);
...
#[derive(Clone, Debug, PartialEq)]
pub struct AllBorderCoordinates(
    pub Vec<Vec<BorderCoordinates>>
);
...
impl AllBorderCoordinates {
    ...
    pub fn new(format: &layout::Format, amount: usize) -> Self {
        ...
        for row in 0..format.rows {
            ...
            for col in 0..format.columns {
                inner.push(BorderCoordinates::new(&format.field_container[row][col], amount));
            }
            vec.push(inner);
        }
    ...
    /// 2 = bottom
    /// 3 = right
    pub fn tesselate(&mut self, format: &layout::Format) {
        for row in 0..format.rows {
            for col in 0..format.columns {
                if row == 0 && col != 0 {
