...
impl Ellipse {
    ...
    pub fn contains(&self, point: Point) -> bool {
        ...
        if ((point.x - self.center.x).powi(2) / (self.radius_x.powi(2))) + ((point.y - self.center.y).powi(2) / (self.radius_y.powi(2))) <= 1.0 {
            true
        } else {
            println!("false");
            false
    ...
    pub fn intersection(&self, line: Line, step: f32) -> Option<Point> {
        ...
        for i in (0..iter_max).map(|x| x as f32 * step){
            ...
            if let Some(point_1) = line.return_point_on_line(x) {
                ...
                if self.center.x > line.start.x {
                    let number = ((point_1.x - self.center.x).powi(2) / (self.radius_x.powi(2))) + ((point_1.y - self.center.y).powi(2) / (self.radius_y.powi(2)));
                    ...
                    if self.contains(point_1) == false {
                        if ((point_1.x - self.center.x).powi(2) / (self.radius_x.powi(2))) + ((point_1.y - self.center.y).powi(2) / (self.radius_y.powi(2))) > 0.5 {
                            ...
                            //     return Some(point)
                            // }                           
                        } else if ((point_1.x - self.center.x).powi(2) / (self.radius_x.powi(2))) + ((point_1.y - self.center.y).powi(2) / (self.radius_y.powi(2))) < 1.5 {
                            return self.intersection(line, step/10.0);
                        }
                    } 
                } else {
